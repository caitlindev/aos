...
MapSteps = function () {
  ...
  this.fillIn = function (field, text, self) {
    var XPath = self.namedSelectors.getXPath('field', field);
    return mapSteps.typeIntoXPathComponent(text, XPath, self);
  };

  this.iPress = function (button, self) {
    var XPath = self.namedSelectors.getXPath('button', button);
    return mapSteps.clickXPathComponent(XPath, self);
  };
  ...
  this.clickElementContaining = function (element, text, self) {
    var XPath = self.namedSelectors.getXPath('tagname_content', text, element);
    return mapSteps.clickXPathComponent(XPath, self);
  };

  this.clickComponentContaining = function (text, self) {
    var XPath = self.namedSelectors.getXPath('node_with_content', text);
    return mapSteps.clickXPathComponent(XPath, self);
  };

  this.clickElementNameId = function (element, nameId, self) {
    var XPath = self.namedSelectors.getXPath('tagname_name_id', nameId, element);
    return mapSteps.clickXPathComponent(XPath, self);
  };

  this.clickComponentNameId = function (nameId, self) {
    var XPath = self.namedSelectors.getXPath('component_name_id', nameId);
    return mapSteps.clickXPathComponent(XPath, self);
  };

  this.typeIntoElementNameId = function (text, element, nameId, self) {
    var XPath = self.namedSelectors.getXPath('tagname_name_id', nameId, element);
    return mapSteps.typeIntoXPathComponent(text, XPath, self);
  };

  this.typeIntoComponentNameId = function (text, nameId, self) {
    var XPath = self.namedSelectors.getXPath('component_name_id', nameId);
    return mapSteps.typeIntoXPathComponent(text, XPath, self);
  };

  this.searchTextAndClickText = function (searchText, clickText, self) {
    var XPath1 = self.namedSelectors.getXPath('node_with_content', searchText),
        XPath2 = self.namedSelectors.getXPath('node_with_content', clickText);
    return mapSteps.searchAndClick(XPath1, XPath2, self);
  };

  this.searchTextAndClickAttribute = function (searchText, attrValue, self) {
    var XPath1 = self.namedSelectors.getXPath('node_with_content', searchText),
        XPath2 = self.namedSelectors.getXPath('node_with_attribute', attrValue);
    return mapSteps.searchAndClick(XPath1, XPath2, self);
  };

  this.searchAndClick = function (XPath1, XPath2, self) {
    ...
    return Q.Promise(function (resolve, reject, notify) {
      ...
      function loopForElement() {
        browser.waitForElementByXPath(XPath1, function (err, el) {
          if (err || !el) {
      ...
      function clickableEle(el) {
        el.elementByXPath(XPath2, function (err, ele) {
          if (err || !ele) {
  ...
  this.clickXPathComponent = function (XPath, self) {
    var browser = self.browser;
    return Q.Promise(function (resolve, reject, notify) {

      browser.waitForElementByXPath(XPath, function (err, el) {
        if (err || (el == null)) {
          err = err ? err : new Error('Did not find component');
          reject(err);
        } else {
  ...
  this.typeIntoXPathComponent = function (text, XPath, self) {
    var browser = self.browser;
        ...
        if (err || (el == null)) {
          ...
          el.clear(function (err) {
            if (err) {
              ...
              browser.type(el, text, function (err) {
                if (err) {
                  reject(err);
                } else {
                  resolve();
  ...
  this.attachFile = function (file, element, self) {
    ...
    var absolutePath = currentPath + "/" + file;
    ...
    return Q.Promise(function (resolve, reject, notify) {
      self.browser.waitForElementByXPath(XPath, function (err, el) {
        if (helper.exists(el)) {
          // waitForElementEnabled clicks on things to make sure its
          // enabled already
          el.sendKeys(absolutePath, function (err) {
            if (err) {
              reject(err);
            } else {
              resolve();
